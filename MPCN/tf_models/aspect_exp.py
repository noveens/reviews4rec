...
class ExpModel:
    ...
    def learn_single_repr(self, q1_embed, q1_len, q1_max, rnn_type,
        ...
        elif('ASP' in rnn_type):
            q1_output, q1_attn_outputs = build_raw_aspect(q1_embed,
                hdim=self.args.rnn_size,
                num_aspect=self.args.num_aspect,
                filter_size=3,
                initializer=self.initializer, 
                dropout=self.rnn_dropout, name=name, reuse=reuse)

            return q1_output, q1_attn_outputs
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            q1_output, q1_output_attn = self.learn_single_repr(self.q1_embed, q1_len, self.qmax,
                                            rnn_type,
                                            reuse=None, 
                                            pool=True,
            ...
            pair_user_q2_output, pair_user_q2_output_attn = self.learn_single_repr(self.pair_user_q2_embed, self.pair_user_len2, self.pair_user_q2max,
                                            rnn_type,
                                            reuse=True, 
                                            pool=True,
            ...
            # experience 
            print(q1_output.shape)
            print(pair_user_q2_output.shape)

            self.exp_output_pos = tf.reshape(q1_output, [-1, self.args.num_aspect * self.args.rnn_size]) # batch_size x num_aspect x rnn_size
            print(self.exp_output_pos.shape)
            self.exp_output_pos = ffn(self.exp_output_pos, self.args.hdim,
                            self.initializer,
                            name='ffn', reuse=None,
                            dropout=self.dropout,
            ...
            self.exp_output_neg = tf.reshape(pair_user_q2_output, [-1, self.args.num_aspect * self.args.rnn_size]) # batch_size x num_aspect x rnn_size
            print(self.exp_output_neg.shape)
            self.exp_output_neg = ffn(self.exp_output_neg, self.args.hdim,
                            self.initializer,
                            name='ffn', reuse=True,
                            dropout=self.dropout,
                            activation=tf.nn.relu, num_layers=1)
