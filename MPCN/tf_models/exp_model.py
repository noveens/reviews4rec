...
class ExpModel:
    ...
    def _get_point_feed_dict(self, data, mode='training', lr=None):
        ...
        #exp_labels = data[-2] # experience ranking
        ...
        if('EXP' in self.args.rnn_type):
            ...
            feed_dict[self.pair_user_inputs1] = data[self.imap['pair_user_inputs1']]
            feed_dict[self.pair_user_len1] = data[self.imap['pair_user_len1']]
            ...
            #feed_dict[self.exp_labels] = exp_labels
    ...
    def prepare_inputs(self):
        ...
        if('EXP' in self.args.rnn_type):
            ...
            pair_user_q1_embed = tf.nn.embedding_lookup(self.embeddings,
                                                        p_user_inputs1)
            pair_user_q2_embed = tf.nn.embedding_lookup(self.embeddings,
                                                        p_user_inputs2)

            self.pair_user_q1_embed = tf.nn.dropout(pair_user_q1_embed, self.emb_dropout)
            self.pair_user_q2_embed = tf.nn.dropout(pair_user_q2_embed, self.emb_dropout)

            self.pair_user_q1_mask = tf.cast(p_user_inputs1, tf.bool)
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.pair_user_q1max, self.pair_user_q2max = self.args.qmax, self.args.qmax
            # if('EXP' in self.args.rnn_type):
            # user_id
            ...
            # add pair input
            with tf.name_scope('pair_user_input1'):
                self.pair_user_inputs1 = tf.placeholder(tf.int32, shape=[None,
                                                    self.pair_user_q1max],
                                                    name='pair_user_inputs1')
            ...
            with tf.name_scope('pair_user_lengths1'):
                self.pair_user_len1 = tf.placeholder(tf.int32, shape=len_shape)
            ...
            _, pair_user_q1_output = self.learn_single_repr(self.pair_user_q1_embed, self.pair_user_len1, self.pair_user_q1max,
                                            rnn_type,
                                            reuse=True, 
                                            pool=True,
                                            name='main', mask=self.pair_user_q1_mask)
            ...
            print(q1_output)
            print(pair_user_q1_output)
            print(pair_user_q2_output)
            ...
            # experience
            exp_output_pos = ffn(pair_user_q1_output, self.args.hdim,
                            self.initializer,
                            name='ffn', reuse=None,
                            dropout=self.dropout,
                        ...
                        if('EXP' in self.args.rnn_type):
                            self.exp_cost = tf.reduce_mean(
                                        -tf.log(tf.nn.sigmoid(
                                            self.exp_output_pos-self.exp_output_neg)))
