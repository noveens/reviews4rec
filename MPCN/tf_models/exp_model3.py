...
class ExpModel:
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            '''self.exp_output_pos = ffn(q1_output, 1,
                            self.initializer,
                            name='ffn', reuse=None,
                            dropout=self.dropout,
            ...
            '''self.exp_output_neg = ffn(pair_user_q2_output, 1,
                            self.initializer,
                            name='ffn', reuse=True,
                            dropout=self.dropout,
                            activation=tf.nn.relu, num_layers=1)
                        ...
                        if('EXP' in self.args.rnn_type):
                            ...
                            self.exp_cost = tf.reduce_mean(
                                        -tf.log(tf.nn.sigmoid(
                                            (self.exp_output_pos-self.exp_output_neg) * tf.expand_dims(self.exp_labels, 1))))
                            self.cost = self.r_cost + lambda1 * self.exp_cost
