...
class Experiment(object):
    ...
    def _load_embeddings(self):
        ...
        if(self.args.pretrained == 1):
            embedding = np.load('./text-recsys/datasets/{}/emb_{}_{}.npy'.format(
                self.args.dataset, self.args.emb_type, # emb_type - glove|fasttext
                self.args.emb_size))
            print(embedding)
    ...
    def _show_metrics(self, epoch, eval_list, show_metrics, name):
        ...
        get_last = eval_list[epoch] # only take the result of that epoch
    ...
    def _select_test_by_dev(self, epoch, eval_dev, eval_test,
        ...
        max_epoch = sorted_test[0][0] # best performance on test set 
